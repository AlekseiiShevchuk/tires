...
@section('content')
    ...
    {!! Form::model($car_model, ['method' => 'PUT', 'route' => ['car_models.update', $car_model->id]]) !!}
        ...
        <div class="panel-body">
            ...
            <div class="row">
                <div class="col-xs-12 form-group">
                    ...
                    {!! Form::select('tires[]', $tires, old('tires') ? old('tires') : $car_model->tires->pluck('id')->toArray(), ['class' => 'form-control select2', 'multiple' => 'multiple']) !!}
                    <p class="help-block"></p>
                    @if($errors->has('tires'))
                        <p class="help-block">
                            {{ $errors->first('tires') }}
        ...
        <div class="panel-body">
            <table class="table table-bordered table-striped">
                ...
                <tbody id="car-numbers">
                    @forelse(old('car_numbers', []) as $index => $data)
                        @include('car_models.car_numbers_row', [
                            'index' => $index
                        ])
                    @empty
                        @foreach($car_model->carNumber as $item)
                            @include('car_models.car_numbers_row', [
                                'index' => 'id-' . $item->id,
                                'field' => $item
                            ])
